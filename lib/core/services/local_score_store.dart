...
///   - `stats:<playerId>` → PlayerStats.toMap() 의 jsonEncode
///   - `h2h:<pairKey>`    → HeadToHead.toMap() 의 jsonEncode
///
...
class LocalScoreStore implements ScoreStore {
  ...
  Future<void> _writeH2h(SharedPreferences p, HeadToHead h) async {
    await p.setString(_h2hKey(h.pairKey), jsonEncode(h.toMap()));
    _h2hCtrls[h.pairKey]?.add(h);
  }
