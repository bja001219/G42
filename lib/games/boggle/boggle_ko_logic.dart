...
///
/// 경로 검증/인접 판정은 [BoggleLogic]을 그대로 재사용한다(4x4 격자, 한 칸 한 글자).
/// 무작위 음절판은 실제 단어가 거의 안 나오므로, 사전에서 뽑은 단어를 인접 경로에
/// '심어서(plant)' 찾을 수 있는 단어를 보장한다.
abstract class KoBoggleLogic {
  static const int size = BoggleLogic.size; // 4
  static const int cellCount = BoggleLogic.cellCount; // 16
  ...
  static String randomBoard(Random rng) {
    ...
    final grid = List<String?>.filled(cellCount, null);

    final target = 4 + rng.nextInt(3); // 4~6개 단어
    var planted = 0;
    var attempts = 0;
    while (planted < target && attempts < 400) {
      attempts++;
      final word = pool[rng.nextInt(pool.length)];
      final path = _tryPlacePath(grid, word.length, rng);
  ...
  static List<int>? _tryPlacePath(List<String?> grid, int len, Random rng) {
    final empties = <int>[
      for (var i = 0; i < cellCount; i++)
        if (grid[i] == null) i,
    ];
  ...
  static bool canFormWord(String grid, String word) =>
      BoggleLogic.canFormWord(grid, word);

  /// 제출 검증. 한글은 2음절 이상.
  static WordCheck check(String grid, String word, Set<String> alreadyFound) {
    final w = word.trim();
    ...
    if (!canFormWord(grid, w)) return WordCheck.notOnBoard;
